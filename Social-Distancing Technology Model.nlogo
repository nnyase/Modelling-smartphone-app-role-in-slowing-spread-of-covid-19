globals
[
  cumulative_infected   ;;The number of total infections, regardless of curity.
  r0                    ;; The reproduction rate of the agents.
]
...
[
  ...
  unisolate?           ;;If true, the agent is unisolated.

  isolation-tick       ;; The agent will cure after 14 days of isolation.
  infection-length     ;; How long the person has been infected

  isolation-tendency   ;; Chance that Normal Users or Initial Infected agents will self-quarantine during any hour being infected.
...
to setup-people
  ...
  create-turtles Initial-Infected [
...
;assign-tendency
to assign-tendency
  set isolation-tendency random-normal average-isolation-tendency
  average-isolation-tendency / 4
...
;;assign color for infected and cured agents.
...
;;go procedure for agents
to go

  ;;limit set if all agents are infected.
  if all? turtles [infected?]
    [ stop ]

  ;;limit set if all agents are cured.
  if all? turtles [cured?]
    [ stop ]

  ;;limit set if no agents are infected (but not cured).
  ...
  ;; procedure to allow Normal Users a chance to isolate.
  ask turtles
   [ if not isolate? and infected? and not strategy? and not cured? and (random 100 < isolation-tendency)
        [ isolate ] ]
  ...
  ;;procedure for infected agents.
  ask turtles
   [ let infected-neighbors count other turtles with [infected?] in-radius 0.2
    if infected-neighbors > 0 [
    set infected? true
    assign-color
    set infection-length 24 + ( random-normal infection-length 5.0 ) ;; mean of infection length and standard-deviation multiplied by 24 hours
...
to move
  ...
  ;;restricting the entering of isolated areas.
  let front-patches patches in-cone 3 60
  if front-patches = white [set heading heading - 180]
...
"" ""
PENS
"Covid- 19 +" 1.0 0 -2674135 true "" "plot count turtles with [infected?]"
"Not Infected" 1.0 0 -13345367 true "" "plot count turtles with [not infected?]"
"Cured agents" 1.0 0 -10899396 true "" "plot count turtles with [cured?]"
"Isolated agents" 1.0 0 -955883 true "" "plot count turtles with [isolate?]"
...
<experiments>
  <experiment name="" repetitions="1" runMetricsEveryStep="false">
    ...
    <metric>count turtles with [isolate?]</metric>
    <metric>ticks</metric>
